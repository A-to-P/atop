...
def signup_restaurant(request):
    if request.method == "POST":
        ...
        if not username or not password:
            ...
            return render(request, 'signup_restaurant.html', {'error': error_username})
        if User.objects.filter(username=username).exists():
            error_username = '이미 사용중인 아이디입니다.'
            return render(request, 'signup_restaurant.html', {'error': error_username})
        if password != password_check:
            error_password = '비밀번호와 비밀번호 확인이 일치하지 않습니다.'
            return render(request, 'signup_restaurant.html', {'error': error_password})
        else:   
            user = User(username=username,password=password,email=email,job='restaurant')
            user.save()
...
def restaurant_field(request):
    name = request.GET.get('restaurant_field')
    value = Tag.objects.filter(restaurant_field=name)    
    job = request.GET.get('signup_restaurant')
    return render(request, 'signup_restaurant.html', {'restaurant_field' : restaurant_field})